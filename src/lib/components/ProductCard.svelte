...
>
  ...
  <div class="aspect-square bg-cream-50 relative overflow-hidden">
    ...
    <!-- Source Badge -->
    {#if source}
      <div class="absolute top-2 right-2 bg-coffee-700 text-cream-100 px-2 py-1 rounded-md text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        {source}
      </div>
    {/if}
  </div>
  ...
  <!-- Product Info -->
  <div class="p-4">
    <h3 class="text-sm font-medium text-coffee-800 mb-2 line-clamp-2 group-hover:text-coffee-900 transition-colors">
      {title || 'Product Title'}
    </h3>
    ...
    <div class="flex items-center justify-between">
      <span class="text-sm text-coffee-600 group-hover:text-coffee-700 transition-colors">
        Click to view
      </span>
