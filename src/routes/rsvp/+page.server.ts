...
const rsvpSchema = z.object({
  ...
  attendeeNames: z.array(z.string().min(1)).min(1, 'At least one attendee name is required'),
...
export const actions = {
  default: async ({ request }) => {
    ...
    const data = {
      ...
      attendeeNames: formData.getAll('attendeeNames').filter(name => name) as string[],
      attendanceCount: parseInt(formData.get('attendanceCount') as string) || 0,
      hasChildren: formData.get('hasChildren') === 'on',
      childrenNames: formData.getAll('childrenNames').filter(name => name) as string[],
    ...
    try {
      ...
      console.log('Dietary Restrictions:', validatedData.dietaryRestrictions || 'None specified');
      console.log('Favorite Coffee:', validatedData.favoriteCoffee || 'Not specified');
      console.log('Favorite Song:', validatedData.favoriteSong || 'Not specified');
